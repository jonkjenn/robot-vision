...
#define UVC_STREAM_FID	(1 << 0)
    ...
    USBMgr::USBMgr()
    {
        debug2("USBMgr create\n");
        int error = libusb_init(&usb_context);
        debug2("libusb error: %d\n", error);
        libusb_set_debug(usb_context, 3);
    }

    USBMgr::~USBMgr()
    {
        debug2("USBMgr destructor called\n");
    ...
    public:
        ...
        void frame_add(enum gspca_packet_type packet_type, const uint8_t *data, int len)
        {
            ...
            if (len > 0)
            {
                ...
                {
                    debug2("discard packet per size %d\n",frame_data_len);
        ...
        void pkt_scan(uint8_t *data, int len)
        {
            ...
            do {
                ...
                if (data[1] & UVC_STREAM_ERR) {
                    //debug2("payload error\n");
                    goto discard;
                }

                /* Extract PTS and FID */
                if (!(data[1] & UVC_STREAM_PTS)) {
                    debug2("PTS not present\n");
                    goto discard;
                }
    ...
    static void LIBUSB_CALL cb_xfr(struct libusb_transfer *xfr)
    {
            ...
            // }
                if(urb->is_streaming)
               {
                    if (libusb_submit_transfer(xfr) < 0) {
                        debug2("error re-submitting URB\n");
                        urb->close_transfers();
                    }
               }
            }
        ...
        if (xfr->type == LIBUSB_TRANSFER_TYPE_ISOCHRONOUS) {
         ...
         //debug2("length:%u, actual_length:%u\n", xfr->length, xfr->actual_length);
    ...
    {
        debug2("PS4EYECAM release called\n");
    ...
    {
        int i;
        debug2("BEGIN MULTIWRITE USB brequest a7\n");
    ...
    void PS4EYECam::multi_register_write(const uint16_t data[][2], uint8_t len, uint8_t subaddr)
    {
        ...
        int i;
        debug2("BEGIN MULTIWRITE USB brequest a4\n");
        /* int size= ARRAY_SIZE(data);
         debug2("size es %d\n",size);*/
        uint8_t *buffer_out=new unsigned char[len*4+16]();
        switch (subaddr) {
    ...
    void PS4EYECam::dump_registers(uint16_t start,uint16_t finish,uint8_t subaddr)
    {
        ...
        uint8_t val=0;
        debug2("READ register BEGIN from 0x%04x - 0x%04x\n ",start,finish);
        while(start<=finish)
        {
            val=register_read(start,subaddr);
            if(val>=0)
            {
                debug2("READ register 0x%04x: 0x%02x\n ",start,val);
            }
            else
            {
                debug2("READ register 0x%04x: READ ERROR ",start);
            }
            //   sleep(1);
            start++;
        }
        debug2("READ FROM SUBADDR 0x%02x END \n",subaddr);
    }
    /*
    ...
    void PS4EYECam::multi_register_read(const uint16_t (*data),uint8_t len, uint8_t subaddr)
    {
        ...
        for(i=0;i<len;i++)
        {
            ...
            {
                debug2("READ register 0x%04x: READ ERROR ",data[i]);
            }
            // sleep(1);
        }
    }
    ...
    {
        //debug2("reg: 0x%02x, val: 0x%02x", reg, val);
    ...
    uint8_t PS4EYECam::register_read(uint16_t reg,uint8_t subaddr)
    {
        ...
        debug2("reg 0x%04x\n", reg);
        debug2("subaddr 0x%04x\n",subaddr);
        /*	for(i=0;i<20;i++)
         {
         debug2("antes %d: 0x%02x \n",i,buffer_out[i]);
         }*/
        switch (subaddr) {
        ...
        /*for(i=0;i<20;i++)
         {
         debug2("post %d: 0x%02x \n",i,buffer_out[i]);
         }

         for(i=0;i<20;i++)
         {
         debug2("prein %d: 0x%02x \n",i,buffer_out[i]);
         }*/
        ...
        if(aux!=reg)
        {
            debug2("phase 2 read failed register read  0x%04x not 0x%04x \n",aux,reg);

        }
          /*for(i=0;i<20;i++)
         {
             debug2("buffer_in %d: 0x%02x \n",i,buffer_in[i]);
         }*/
           //debug2("read  register 0x%04x  value 0x%02x \n",reg,buffer_in[18]);
        return buffer_in[18];
    ...
    {
        debug2("stop_sensor_streaming called\n");
    ...
    void PS4EYECam::dump_sensor_generalconf(uint8_t n)
    {
        ...
        {
            case 1:
                debug2("Dumping Sensor 1 general conf registers\n");
                break;
            case 2:
                debug2("Dumping Sensor 2 general conf registers\n");
                break;
            default:
                debug2("%d is not a valid sensor number\n",n);
                return;
                break;
        }
    ...
    void PS4EYECam::set_sensor_video_mode(uint8_t mode, uint8_t n)
    {
        ...
        switch(mode)
        {
            ...
            case 28:
                ...
                break;
            default:
                debug2("%d is not a valid mode number\n",mode);
                break;
        }
    ...
    bool PS4EYECam::read_sensor_id(uint8_t n)
    {
        ...
        debug2("read sensor_id from sensor %d\n",n);

        sensor_id_h=register_read(OV9713_CHIP_ID_HI, n);
        debug2("sensor_id_h %d\n", sensor_id_h);
        sleep(1);
        sensor_id_l=register_read(OV9713_CHIP_ID_LO, n);
        debug2("sensor_id_l %d\n", sensor_id_h);
        sensor_id=sensor_id_h<<8|sensor_id_l;
        debug2("sensor %d id 0x%04x \n",n,sensor_id);
        if(sensor_id!=0x9713)
        {
            debug2("Sensor %d is not ready, exit\n",n);
           // stop_sensors_streaming();
           // reset_sensors();
            //put led off
            //set_led_off();
    ...
    bool PS4EYECam::init(uint32_t width, uint32_t height, uint8_t desiredFrameRate)
    {
        ...
        if(!check_sensors())
        {
            debug2("Sanity check from sensors failed\n");
            //set_led_off();
            //reset_sensors();
            return false;
        }
    ...
    /*  TODO play with video modes perhaps we will need to go to uvc path without ov580 information.
        debug2("Show video mode registers from sensor 1\n");
        dump_sensor_video_mode(1);
        debug2("Show video mode registers from sensor 2\n");
        dump_sensor_video_mode(2);
...
/*
        if(!init_registers())
        {
            debug2("Initialization registers failed\n");
            return false;
        }
*/
        return true;
    ...
    void PS4EYECam::check_ff71()
    {
        ...
        switch(urb->ff71status)
        {
            case 0:
                    ...
                    if(val==0x0)
                    {
                        debug2("i=%d Write register 0xff70 to f1\n",urb->ff71status);
                     //   usleep(1);
            ...
            case 1:
                    ...
                    debug2("i=%d Write register 0xff70 to 01\n",urb->ff71status);
  ...
  /*  const uint8_t* PS4EYECam::getLastVideoFramePointer()
    ...
    {
        debug2("     USB 3.0 Endpoint Companion:\n");
        debug2("       bMaxBurst:        %d\n", ep_comp->bMaxBurst);
        debug2("       bmAttributes:     0x%02x\n", ep_comp->bmAttributes);
        debug2("       wBytesPerInterval: %d\n", ep_comp->wBytesPerInterval);
    }
    ...
    void PS4EYECam::dump_endpoint(const struct libusb_endpoint_descriptor *endpoint)
    {
        ...
        debug2("     Endpoint:\n");
        debug2("       bEndpointAddress: %02xh\n", endpoint->bEndpointAddress);
        debug2("       bmAttributes:     %02xh\n", endpoint->bmAttributes);
        debug2("       wMaxPacketSize:   %d\n", endpoint->wMaxPacketSize);
        debug2("       bInterval:        %d\n", endpoint->bInterval);
        debug2("       bRefresh:         %d\n", endpoint->bRefresh);
        debug2("       bSynchAddress:    %d\n", endpoint->bSynchAddress);
    ...
    {
        int i;
        debug2(" Configuration Descriptor:\n"
              "   bLength             %5u\n"
              "   bDescriptorType     %5u\n"
              "   wTotalLength        %5u\n"
              "   bNumInterfaces      %5u\n"
        ...
        if (!(config->bmAttributes & 0x80))
            debug2("    (Missing must-be-set bit!)\n");
        if (config->bmAttributes & 0x40)
            debug2("    Self Powered\n");
        else
            debug2("    (Bus Powered)\n");
        if (config->bmAttributes & 0x20)
            debug2("  Remote Wakeup\n");
        if (config->bmAttributes & 0x10)
            debug2("  Battery Powered\n");
        debug2("   MaxPower            %5umA\n", config->MaxPower * 2);
        if (config->extra_length) {
            const unsigned char	*buf = config->extra;
            if(buf[1]==0x0b)
            {
                debug2("   Interface Association:\n"
                      "     bLength %5u\n"
                      "     bDescriptorType   %5u\n"
                      "     bFirstInterface   %5u\n"
                      "     bInterfaceCount   %5u\n"
    ...
    void PS4EYECam::dump_device()
    {
        ...
        if(res)
        {
            debug2("error getting device descriptor: %d\n", res);
            return;
        }
        debug2("Device Descriptor:\n"
              " bLength            %5u\n"
              " bDescriptorType    %5u\n"
              " bcdUSB             %2x.%02x\n"
              " bDeviceClass       %5u \n"
        ...
        {
            debug2("error getting config descriptors\n");
            return;
        }
        dump_config(config);
   	...
   	int PS4EYECam::uvc_set_video_mode(uint8_t mode,uint8_t fps)
	{
		...
		switch(mode)
		{
			case 0://3448x808
				...
				{
					debug2("Unsupported fps=%d take default 60 fps\n",fps);
					frame_rate=60;
				}
				else
				{
		...
		debug2("Frame resolutiion: %dx%d framerate: %d \n",frame_width,frame_height,frame_rate);
		ctrl.bmHint=1;
		ctrl.bFormatIndex=1;
        ...
        if (err == len)
        {
            debug2("control transfer for video set return %d\n",err);
            return 0;
        }
        else
        {
            debug2("control transfer for video set error return %d\n",err);
            return err;
        }
	}
    int PS4EYECam::uvc_show_video_mode()
    {
        ...
        if (err == len)
        {
            debug2("control transfer for video get return %d\n",err);
            ...
            debug2("bmHint: %04x\n", ctrl.bmHint);
            debug2("bFormatIndex: %d\n", ctrl.bFormatIndex);
            debug2("bFrameIndex: %d\n", ctrl.bFrameIndex);
            debug2("dwFrameInterval: %u\n", ctrl.dwFrameInterval);
            debug2("wKeyFrameRate: %d\n", ctrl.wKeyFrameRate);
            debug2("wPFrameRate: %d\n", ctrl.wPFrameRate);
            debug2("wCompQuality: %d\n", ctrl.wCompQuality);
            debug2("wCompWindowSize: %d\n", ctrl.wCompWindowSize);
            debug2("wDelay: %d\n", ctrl.wDelay);
            debug2("dwMaxVideoFrameSize: %u\n", ctrl.dwMaxVideoFrameSize);
            debug2("dwMaxPayloadTransferSize: %u\n", ctrl.dwMaxPayloadTransferSize);
            debug2("bInterfaceNumber: %d\n", ctrl.bInterfaceNumber);
        ...
        {
            debug2("control transfer for video ger error return %d\n",err);
            return err;
        }
    ...
    bool PS4EYECam::open_usb()
    {
        ...
        if(res != 0) {
            debug2("set configuration error: %d\n", res);
            return false;
        }
        debug2("configuration set to 1\n");

        //claim interface 1
        res = libusb_claim_interface(handle_, 1);
        if(res != 0) {
            debug2("device claim interface error: %d\n", res);
            return false;
        }
        debug2("claim interface 1\n");
        ...
        if(res != 0) {
            debug2("set alt setting to interface error: %d\n", res);
            return false;
        }
        debug2("set alt to 1 in interface 1\n");
        uvc_show_video_mode();
    ...
    void PS4EYECam::submit_controlTransfer(uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, uint16_t wLength, uint8_t *buffer )
    {
        ...
        debug2("Libusb_submit_transfer %d\n", error);
    }
